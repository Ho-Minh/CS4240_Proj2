...
IRToMIPSSelector::selectProgram(const IRProgram& program) {
    ...
    for (const auto& fn : program.functions) {
        ...
        if (getAllocMode() == AllocMode::Naive) {
            ...
            std::vector<std::pair<int,int>> blocks; // [l,r]
            ...
            // Allocatable registers for greedy mapping (keep t0..t4 for temps)
            ...
            auto emitOpWithRegs = [&](const std::shared_ptr<IROperand>& op,
                ...
                if (auto v = std::dynamic_pointer_cast<IRVariableOperand>(op)) {
                    ...
                    if (it != map.end()) {
                        // Already in a register; move into tmp if tmp is requested by caller
                        if (tmp) code.emplace_back(MIPSOp::MOVE, "", std::vector<std::shared_ptr<MIPSOperand>>{ tmp, it->second });
                        return;
                    }
                }
                // Fallback to memory/immediate load into tmp
            ...
            for (const auto& br : blocks) {
                ...
                // Emit label first if block starts with one
                if (F.instructions[bi]->opCode == IRInstruction::OpCode::LABEL) {
                    ...
                    std::string L = qualLabel(F.name, lbl->getName());
                    out.emplace_back(MIPSOp::SLL, L, std::vector<std::shared_ptr<MIPSOperand>>{ Registers::zero(), Registers::zero(), std::make_shared<Immediate>(0) });
                }

                // 2) Count uses within block
                ...
                // 4) Load mapped vars at block entry
                for (auto& kv : regMap) {
                    int off = fi.varOffset[kv.first];
                    out.emplace_back(MIPSOp::LW, "", std::vector<std::shared_ptr<MIPSOperand>>{ kv.second, std::make_shared<Address>(off, Registers::fp()) });
                }

                // 5) Emit instructions using regMap
                for (int i = bi; i <= bj; ++i) {
                    auto ir = F.instructions[i]; if (!ir) continue;
                    if (ir->opCode == IRInstruction::OpCode::LABEL && i == bi) continue; // already emitted
                    std::vector<MIPSInstruction> code;
                    switch (ir->opCode) {
                        case IRInstruction::OpCode::ASSIGN: {
                            auto dst = std::dynamic_pointer_cast<IRVariableOperand>(ir->operands[0]);
                            // Bulk array set: ASSIGN array, count, value
                            if (ir->operands.size() == 3 && dst && std::dynamic_pointer_cast<IRArrayType>(dst->type)) {
                                ...
                                // Load count and value (prefer mapped if available)
                                ...
                                if (it != regMap.end()) {
                                    ...
                                    if (auto c = std::dynamic_pointer_cast<IRConstantOperand>(ir->operands[1])) {
                                        ...
                                        if (it2 != regMap.end()) {
                                            code.emplace_back(MIPSOp::MOVE, "", std::vector<std::shared_ptr<MIPSOperand>>{ dstR, it2->second });
                                        } else {
                                            auto t0 = Registers::t0();
                                            loadOp(ir->operands[1], t0, code);
                        ...
                        case IRInstruction::OpCode::GOTO: {
                            auto lbl = std::dynamic_pointer_cast<IRLabelOperand>(ir->operands[0]);
                            // Flush mapped registers before control transfer
                            for (auto& kv : regMap) {
                                int off = fi.varOffset[kv.first];
                        ...
                        case IRInstruction::OpCode::BRGEQ: {
                            ...
                            // Flush mapped registers before control transfer so target sees updated memory
                            for (auto& kv : regMap) {
                                int off = fi.varOffset[kv.first];
                                code.emplace_back(MIPSOp::SW, "", std::vector<std::shared_ptr<MIPSOperand>>{ kv.second, std::make_shared<Address>(off, Registers::fp()) });
                            }
                        ...
                        case IRInstruction::OpCode::CALLR: {
                            ...
                            if (callee == "geti") {
                                ...
                                if (ir->opCode == IRInstruction::OpCode::CALLR) {
                                    ...
                                    auto it = regMap.find(dst ? dst->getName() : std::string());
                                    if (it != regMap.end()) {
                                        code.emplace_back(MIPSOp::MOVE, "", std::vector<std::shared_ptr<MIPSOperand>>{ it->second, Registers::v0() });
                                    } else {
                                        storeVar(dst->getName(), Registers::v0(), code);
                                    }
                                }
                            ...
                            // normal call: pass up to 4 args, push extras on stack
                            static std::shared_ptr<Register> aRegs[4] = { Registers::a0(), Registers::a1(), Registers::a2(), Registers::a3() };
                            // Spill mapped registers around call (caller-saved)
                            for (auto& kv : regMap) {
                                int off = fi.varOffset[kv.first];
                                code.emplace_back(MIPSOp::SW, "", std::vector<std::shared_ptr<MIPSOperand>>{ kv.second, std::make_shared<Address>(off, Registers::fp()) });
                            }
                            ...
                            // Reload mapped registers after call
                            for (auto& kv : regMap) {
                                int off = fi.varOffset[kv.first];
                        ...
                        case IRInstruction::OpCode::LABEL:
                        default: {
                            // Use existing naive handlers for these cases
                            switch (ir->opCode) {
                                case IRInstruction::OpCode::LABEL: {
                                    auto lbl = std::dynamic_pointer_cast<IRLabelOperand>(ir->operands[0]);
                                    std::string L = qualLabel(F.name, lbl->getName());
                                    code.emplace_back(MIPSOp::SLL, L, std::vector<std::shared_ptr<MIPSOperand>>{ Registers::zero(), Registers::zero(), std::make_shared<Immediate>(0) });
                                    break;
                                }
                                case IRInstruction::OpCode::ARRAY_STORE: {
                                    auto tVal = Registers::t0();
                                    auto tIdx = Registers::t1();
                                    auto tAddr = Registers::t2();
                                    // Prefer mapped registers in greedy mode
                                ...
                                case IRInstruction::OpCode::ARRAY_LOAD: {
                                    ...
                                    code.emplace_back(MIPSOp::ADD, "", std::vector<std::shared_ptr<MIPSOperand>>{ tAddr, baseReg, tAddr });
                                    code.emplace_back(MIPSOp::LW,  "", std::vector<std::shared_ptr<MIPSOperand>>{ tVal, std::make_shared<Address>(0, tAddr) });
                                    // If dst in regMap, keep in reg; else store
                                    auto it = regMap.find(dst ? dst->getName() : std::string());
                                    if (it != regMap.end()) {
                                        code.emplace_back(MIPSOp::MOVE, "", std::vector<std::shared_ptr<MIPSOperand>>{ it->second, tVal });
                                    } else {
                                        storeVar(dst->getName(), tVal, code);
                                    }
                                    break;
                                ...
                                case IRInstruction::OpCode::CALLR:
                                default:
                                    // Fall back to original implementations (will handle syscalls, params, etc.)
                                    break;
                            }
                            break;
                        }
                ...
                // 6) Store mapped vars at block end (before control transfer)
                if (bj + 1 < (int)F.instructions.size()) {
                    auto next = F.instructions[bj + 1];
                    bool endsWithJump = false;
                    if (next && next->opCode == IRInstruction::OpCode::LABEL) {
                        // conservative: still flush at block boundary
                        endsWithJump = true;
                    }
                    (void)endsWithJump; // not used further, keep conservative flush
                }
                for (auto& kv : regMap) {
                    int off = fi.varOffset[kv.first];
